...
def extract_features():
    ...
    stk_tickers = ['MCD', 'SBUX', 'CMG', 'YUM','DPZ']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'MCD')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('SBUX', 'CMG', 'YUM','DPZ'))]).diff(return_period)
